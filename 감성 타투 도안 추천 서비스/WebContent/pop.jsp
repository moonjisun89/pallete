...
<body>
<%	
	RankingDTO rinfo = (RankingDTO)session.getAttribute("rinfo");
		...
		<div class="layer_cont" style="padding:40px;">
			...
			<div id="tt_detail">
				<div id="slide1">
					...
					<%if (rinfo != null){ %>
						<ul class="sd1">
							<li><img src="<%=rinfo.getShop_img1() %>" width="100%" /></li>
							<li><img src="<%=rinfo.getShop_img2() %>" width="100%" /></li>
							<li><img src="<%=rinfo.getShop_img3() %>" width="100%" /></li>
							<li><img src="<%=rinfo.getShop_img4() %>" width="100%" /></li>
							<li><img src="<%=rinfo.getShop_img5() %>" width="100%" /></li>
						</ul>
					</div>
				</div>
				<div class="tt_dtl_txt">
					<h2><%=rinfo.getShop_name() %></h2>
					<p><%=rinfo.getShop_content() %></p>
					<dl>
						<dt class="ts1">주소</dt><dd><%=rinfo.getShop_addr() %></dd>
						<dt class="ts2">연락처</dt><dd><%=rinfo.getShop_tel() %></dd>
						<dt class="ts3">운영시간</dt><dd><%=rinfo.getShop_time() %></dd>
						<dt class="ts4">시술종류</dt><dd><%=rinfo.getShop_kinds() %></dd>
					</dl>
					<a href="https://www.instagram.com/" target="_blank">SNS 바로가기</a>
					<a onclick="window.open('./PayController?shop_num=<%=rinfo.getShop_num() %>');" target="_blank" style="background:#54349e">예약하기</a>
